...
def iupred(folder, seq, mode='short'):
    if mode == "short":
        ...
        uc = 25
        wc = 10
        mtx = read_matrix("{}/iupred2_short_energy_matrix".format(folder))
        histo, histo_min, histo_max, histo_step = read_histo("{}/short_histogram".format(folder))

    elif mode == 'glob':
        lc = 1
        uc = 100
        wc = 15
        mtx = read_matrix("{}/iupred2_long_energy_matrix".format(folder))
        histo, histo_min, histo_max, histo_step = read_histo("{}/long_histogram".format(folder))
...
def anchor2(folder, seq, iupred_scores):
    ...
    mtx = read_matrix('{}/anchor2_energy_matrix'.format(folder))
    interface_comp = {}
    with open('{}/anchor2_interface_comp'.format(folder)) as _fn:
        for line in _fn:
